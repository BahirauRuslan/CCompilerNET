...
namespace CCompiler.ObjectDefinitions
{
    ...
    {
        protected static ILGenerator iLGenerator;
		...
		public abstract ObjectScope Scope { get; }
			...
			switch (localVarNumber)
			{
				...
				default:
					if (localVarNumber < 256)
						iLGenerator.Emit(OpCodes.Stloc_S, localVarNumber);
					else
						iLGenerator.Emit(OpCodes.Stloc, localVarNumber);
					break;
			}
		}
		...
		protected static void EmitLoadFromLocal(int localVarNumber)
		{
			switch (localVarNumber)
			{
				...
				default:
					if (localVarNumber < 256)
						iLGenerator.Emit(OpCodes.Ldloc_S, localVarNumber);
					else
						iLGenerator.Emit(OpCodes.Ldloc, localVarNumber);
					break;
			}
		}
